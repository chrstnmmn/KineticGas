document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('containerWidth').addEventListener('input', function () {
    containerWidth = parseInt(this.value)
  ...
  document.getElementById('containerHeight').addEventListener('input', function () {
    containerHeight = parseInt(this.value)
    document.getElementById('heightValue').textContent = containerHeight
    resizeCanvas()
  })
  ...
  function explode() {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      explosionParticles.push(
        new ExplosionParticle(centerX, centerY, `hsl(${Math.random() * 60}, 100%, 50%)`)
      )
    }
  ...
  document.getElementById('temperatureInput').addEventListener('input', function () {
    ...
    const sliderMax = parseInt(document.getElementById('temperature').max)
    const sliderMin = parseInt(document.getElementById('temperature').min)
    ...
    if (!isExploded) {
      ...
      const oldTemperature =
        particles.length > 0 ? particles[0].vx / ((Math.random() - 0.5) / 100) : 300 // Estimate old T if possible
      const scaleFactor = Math.sqrt(temperature / (oldTemperature || 300))
  ...
  document.getElementById('strengthInput').addEventListener('input', function () {
    ...
    // Adjust slider - it might not cover the full range of the input now
    const sliderMax = parseInt(document.getElementById('containerStrength').max)
    const sliderMin = parseInt(document.getElementById('containerStrength').min)
  ...
  document.getElementById('temperature').addEventListener('input', function () {
    ...
    if (!isExploded) {
      const oldTemperature =
        particles.length > 0 ? particles[0].vx / ((Math.random() - 0.5) / 100) : 300
      const scaleFactor = Math.sqrt(temperature / (oldTemperature || 300))
      particles.forEach((p) => {
        p.vx *= scaleFactor
        p.vy *= scaleFactor
